...
@property(nonatomic)  BOOL moveEnd;
...
- (void)awakeFromNib {
    [super awakeFromNib];
    self.style = CCDraggableStyleDownOverlay;
}
...
- (void)panGestureHandle:(UIPanGestureRecognizer *)gesture
{
    ...
    if (gesture.state == UIGestureRecognizerStateChanged) {
        ...
        if (self.delegate && [self.delegate respondsToSelector:@selector(draggableContainer:cardView:draggableDirection:horizontalOffset:)])
        {
            /*
    ...
    [self.delegate draggableContainer:self cardView:cardView draggableDirection:self.direction horizontalOffset:gesture.view.center.x - self.cardCenter.x];
}
...
//                             [self.delegate draggableContainer:self draggableDirection:self.direction didSelectIndex:(NSInteger)cardView.tag move:self.moveEnd];
                         }];
        if ([self.delegate respondsToSelector:@selector(draggableContainer:draggableDirection:didSelectIndex:move:)]) {
            [self.delegate draggableContainer:self draggableDirection:self.direction didSelectIndex:(NSInteger)cardView.tag move:self.moveEnd];
        }
...
//                         if (weakself.delegate && [weakself.delegate respondsToSelector:@selector(draggableContainer: draggableDirection: finishedDraggableLastCard:)]) {
//                             if (weakself.currentCards.count == 0) {
//                                 [weakself.delegate draggableContainer:self draggableDirection:self.direction finishedDraggableLastCard:YES];
//                             }
//                         }
                     }];
    if (self.currentCards.count == 0 &&self.delegate && [self.delegate respondsToSelector:@selector(draggableContainer: draggableDirection: finishedDraggableLastCard:)]) {
        [self.delegate draggableContainer:self draggableDirection:self.direction finishedDraggableLastCard:YES];
    }
}
...
//    if (self.delegate && [self.delegate respondsToSelector:@selector(draggableContainer:cardView:draggableDirection:horizontalOffset:)]) {
//        [self.delegate draggableContainer:self cardView:nil draggableDirection:self.direction horizontalOffset:0];
//    }
