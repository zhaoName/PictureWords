...
- (instancetype)init
{
    self = [super init];
    if (self) {
        NSString *basePath = [[NSBundle mainBundle] pathForResource:@"ZZLearnWords" ofType:@"bundle"];
        NSString *englishPath = [basePath stringByAppendingPathComponent:@"EnglishData.plist"];
        NSString *daanPath = [basePath stringByAppendingPathComponent:@"suoyoudaan.plist"];
        self.mainData = [[NSArray alloc] initWithContentsOfFile:englishPath];
        NSDictionary *dic = [[NSDictionary alloc] initWithContentsOfFile:daanPath];;
